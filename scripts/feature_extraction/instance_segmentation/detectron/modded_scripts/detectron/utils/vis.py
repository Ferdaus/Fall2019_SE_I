...
def vis_one_image_Tarik(
    ...
    if not idxs:
        print("###############")
        print("No person objects detected with confidence higher than the threshold")
        print("###############")
        return
    #thresh = 0.5
    ...
    fig2.savefig(os.path.join(output_dir, '{}'.format(output_name_mask)), dpi=dpi)
    
    output_name3 = os.path.basename(im_name) + '_masked.' + ext
    # #fig3.savefig(os.path.join(output_dir, '{}'.format(output_name3)), dpi=dpi)
    plt.imsave(os.path.join(output_dir, '{}'.format(output_name3)), im_masked, cmap='gray')
    #print(image.shape)
...
def vis_one_image_Tarik_skeleton(
        im, im_name, output_dir, boxes, segms=None, keypoints=None, thresh=0.9,
        kp_thresh=2, dpi=200, box_alpha=0.0, dataset=None, show_class=False,
        ext='pdf', out_when_no_box=False, is_color_skeleton = False, color_skeleton = 'w'):
    """Visual debugging of detections."""
    ...
    fig2.savefig(os.path.join(output_dir, '{}'.format(output_name_skeleton)), dpi=dpi)
    
    output_name_im_skeleton = os.path.basename(im_name) + '_im_skeleton.' + ext
    ax2.imshow(im)
    fig2.savefig(os.path.join(output_dir, '{}'.format(output_name_im_skeleton)), dpi=dpi)
    
    output_name_keypoints = os.path.basename(im_name) + '_keypoints.txt' 
    with open(os.path.join(output_dir, '{}'.format(output_name_keypoints)),'w') as f:
