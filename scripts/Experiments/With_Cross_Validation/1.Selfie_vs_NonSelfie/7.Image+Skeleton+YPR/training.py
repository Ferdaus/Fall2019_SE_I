...
def train_hybrid(train_idxs, val_idxs, train_val_df, image_paths, feature_config_label, feature_config, network_architecture, save_file_suffix, do_callbacks = True ):
    ...
    learn = ImageTabularLearner(data_fold, model, metrics=[error_rate, accuracy], callback_fns = learner_callbacks)
    ...
    learn.lr_find()
    learn.recorder.plot()
    learn.fit_one_cycle(1, .4e-3, callbacks = training_callbacks)
    learn.save(f'stage-1'+save_file_suffix)
    ...
    learn.lr_find()
    learn.recorder.plot()
    learn.fit_one_cycle(1, max_lr=slice(1.1e-6,1.1e-6/3), callbacks = training_callbacks)
